...
def _vslerp(v0: np.ndarray, v1: np.ndarray, t: float) -> np.ndarray:
    ...
    if d < -0.9999:  # antipodal — route through perpendicular
        cand = (
            np.array([1.0, 0.0, 0.0]) if abs(v0[0]) < 0.9 else np.array([0.0, 1.0, 0.0])
        )
        mid = _norm(np.cross(v0, cand))
        return (
            _vslerp(v0, mid, t * 2.0) if t < 0.5 else _vslerp(mid, v1, (t - 0.5) * 2.0)
        )

    omega = math.acos(d)
    s_o = math.sin(omega)
    return (math.sin((1.0 - t) * omega) / s_o) * v0 + (math.sin(t * omega) / s_o) * v1
...
class NavCubeOverlay(QWidget):
    ...
    _DDEF = _norm(np.array([-1.0, 1.0, -1.0]))
    _UDEF = np.array([0.0, 0.0, 1.0])
    ...
    def __init__(
        self, parent=None, *, overlay: bool = True, style: NavCubeStyle | None = None
    ):
        """
        Parameters
        ──────────
        ...
        self._ready = False  # set True via mark_ready() once the OCC view is live
        ...
        # Relative layout helpers (public API): anchor the cube to a target widget.
        # Must be initialized before _update_dpi(), which may call update_relative_position().
        self._layout_target = parent
        self._layout_anchor = "top-right"
        self._layout_margin = (10, 10)
    ...
    def _add_button_shape(self, name, act):
        ...
        if name in ("ArrowLeft", "ArrowRight"):
            ...
            for idx in range(steps):
                ...
                point_data.extend(
                    [10.0 * math.cos(ang) + 87.0, 10.0 * math.sin(ang) - 87.0]
                )
        else:
            point_data = []
    ...
    def _update_dpi(self) -> None:
        ...
        — when size_fraction > 0 — whenever the layout target resizes.
        ...
        fraction of the layout target's shorter logical side converted to
        ...
        try:
            ...
            layout_target = getattr(self, "_layout_target", None)
            size_target = (
                layout_target if layout_target is not None else self.parentWidget()
            )

            if s.size_fraction > 0.0 and size_target is not None:
                # Viewport-fraction mode ─────────────────────────────────────
                # Work in physical pixels throughout so the fraction is
                # maintained regardless of OS scale / DPI.
                vp_logical = min(size_target.width(), size_target.height())
    ...
    def configure_relative_layout(
        self,
        *,
        target: QWidget | None = None,
        anchor: str = "top-right",
        margin: int | tuple[int, int] = 10,
        size_fraction: float | None = None,
        size_min: int | None = None,
        size_max: int | None = None,
    ) -> None:
        """Configure automatic size/position relative to a target widget.

        Parameters
        ----------
        target:
            Widget whose rectangle is used as the anchor frame. Defaults to parent.
        anchor:
            One of: top-left, top, top-right, left, center, right,
            bottom-left, bottom, bottom-right.
        margin:
            Pixel margin from anchored edges. Int applies to both axes.
        size_fraction:
            Optional viewport fraction of target min(width, height). When set,
            auto-resize is enabled using NavCubeStyle.size_fraction.
        size_min / size_max:
            Optional size bounds for size_fraction mode.
        """
        self.set_layout_target(target)
        self.set_layout_anchor(anchor)
        self.set_layout_margin(margin)

        if size_fraction is not None:
            self.set_size_fraction(size_fraction, size_min=size_min, size_max=size_max)
        elif size_min is not None or size_max is not None:
            if size_min is not None:
                self._style.size_min = int(size_min)
            if size_max is not None:
                self._style.size_max = int(size_max)
            self._update_dpi()

        self.update_relative_position()

    def set_layout_target(self, target: QWidget | None) -> None:
        """Set the widget used as the reference frame for relative positioning."""
        self._layout_target = target if target is not None else self.parentWidget()
        if self.isVisible():
            self._install_parent_filter()
        self.update_relative_position()

    def set_layout_anchor(self, anchor: str) -> None:
        """Set anchor position used by update_relative_position()."""
        key = anchor.strip().lower()
        if key not in self._ANCHORS:
            raise ValueError(f"Unsupported anchor '{anchor}'.")
        self._layout_anchor = key
        self.update_relative_position()

    def set_layout_margin(self, margin: int | tuple[int, int]) -> None:
        """Set margin used by update_relative_position()."""
        if isinstance(margin, tuple):
            if len(margin) != 2:
                raise ValueError("margin tuple must have exactly two integers.")
            mx, my = int(margin[0]), int(margin[1])
        else:
            mx = my = int(margin)
        self._layout_margin = (max(0, mx), max(0, my))
        self.update_relative_position()

    def set_size_fraction(
        self,
        fraction: float,
        *,
        size_min: int | None = None,
        ...
        """Enable or update fraction-based cube sizing relative to target/widget."""
        self._style.size_fraction = max(0.0, float(fraction))
        if size_min is not None:
            self._style.size_min = int(size_min)
        if size_max is not None:
            self._style.size_max = int(size_max)
    ...
    def set_fixed_size_mode(self, size: int | None = None) -> None:
        """Disable fraction sizing and optionally set a fixed reference size."""
        self._style.size_fraction = 0.0
        if size is not None:
            self._style.size = int(size)
        self._update_dpi()

    def update_relative_position(self) -> None:
        """Reposition the cube based on target, anchor and margin."""
        layout_target = getattr(self, "_layout_target", None)
        target = layout_target if layout_target is not None else self.parentWidget()
        if target is None:
            return
        ...
        mx, my = self._layout_margin
        x_map = {
            "left": mx,
            "center": max(0, (tw - cw) // 2),
            "right": max(0, tw - cw - mx),
        }
        y_map = {
            "top": my,
            "center": max(0, (th - ch) // 2),
            "bottom": max(0, th - ch - my),
        }

        anchor = self._layout_anchor
        if anchor == "top-left":
            x, y = x_map["left"], y_map["top"]
        elif anchor == "top":
            x, y = x_map["center"], y_map["top"]
        elif anchor == "top-right":
            x, y = x_map["right"], y_map["top"]
        elif anchor == "left":
            x, y = x_map["left"], y_map["center"]
        elif anchor == "center":
            x, y = x_map["center"], y_map["center"]
        elif anchor == "right":
            x, y = x_map["right"], y_map["center"]
        elif anchor == "bottom-left":
            x, y = x_map["left"], y_map["bottom"]
        elif anchor == "bottom":
            x, y = x_map["center"], y_map["bottom"]
        else:
            x, y = x_map["right"], y_map["bottom"]

        local_pos = QPointF(float(x), float(y)).toPoint()
        parent = self.parentWidget()

        if parent is target and not self.isWindow():
            self.move(local_pos)
        elif self.isWindow():
            self.move(target.mapToGlobal(local_pos))
        elif parent is not None:
            self.move(parent.mapFromGlobal(target.mapToGlobal(local_pos)))
        else:
            self.move(local_pos)
    ...
    def push_camera(
        self, dx: float, dy: float, dz: float, ux: float, uy: float, uz: float
    ) -> None:
        """
    ...
    def _smooth_camera_state(
        self, d_tgt: np.ndarray, u_tgt: np.ndarray, alpha: float
    ) -> bool:
        """
        ...
        if (
            np.linalg.norm(d_tgt - self._dir) <= self._SYNC_EPS
            and np.linalg.norm(u_tgt - self._up) <= self._SYNC_EPS
        ):
            return False
    ...
    def _label_font(self, text: str) -> QFont:
        ...
        if size is None:
            ...
            if bounds.width() > 1e-6 and bounds.height() > 1e-6:
                size = 100.0 * min(
                    target_w / bounds.width(), target_h / bounds.height()
                )
            else:
                size = 50.0
    ...
    def _proj(self, P, R, U, cx, cy) -> QPointF:
        S = self._SCALE
        return QPointF(cx + float(np.dot(P, R)) * S, cy - float(np.dot(P, U)) * S)
    ...
    def mark_ready(self) -> None:
        """Call once the 3-D view is fully initialised and it is safe to paint.
        Until this is called, paintEvent is a no-op so the widget never tries
        to draw before the OCC/OpenGL context is live (which would produce a
        blank first frame or, on Windows, a transient engine==0 warning)."""
        self._ready = True
        self.update()
    ...
    def paintEvent(self, event):  # noqa: N802
        ...
        # Use QImage instead of QPixmap as the intermediate paint buffer.
        # ROOT CAUSE of the "engine == 0, type: 3" / QWinFontEngine errors:
        #   QPixmap on Windows is GDI-backed. When OCC fires view.Redraw()
        #   (triggered by viewOrientationRequested during face-click animation),
        #   the OpenGL/DirectX compositor temporarily locks ALL GDI surfaces.
        #   The font engine then calls GetGlyphOutline() on the locked QPixmap
        #   surface, gets Windows error 1003 (ERROR_PARTIAL_COPY), the painter
        #   engine nulls out, and every subsequent painter call logs
        #   "Painter not active". QImage is plain CPU heap memory — the GL
        #   compositor cannot touch it, so GetGlyphOutline always succeeds.
        ...
        if self._overlay:
            ...
            wp.fillRect(self.rect(), Qt.transparent)
        wp.drawImage(0, 0, img)
        wp.end()
    ...
    def _face_col(self, f, pal) -> QColor:
        ...
        ft = f["ft"]
        base = (
            pal.f_main if ft == "main" else (pal.f_edge if ft == "edge" else pal.f_corn)
        )
        shade = 0.85 + 0.15 * max(0.0, float(np.dot(f["n"], -self._LIGHT)))
        return QColor(
    ...
    def _draw_label(self, p, f, R, U, cx, cy, font, text, col):
        ...
        # Guard: skip label if the projected quad is near-degenerate.
        # ROOT CAUSE of "QWinFontEngine: GetGlyphOutline() failed, error 1003":
        #   During face-click animation a face can be nearly edge-on to the
        #   camera. Its 4 projected corners become nearly collinear, so the
        #   destination quad has near-zero area. quadToQuad() still returns
        #   True but produces a transform that shrinks the 200x200 src to
        #   ~0 pixels. When drawText() asks the Windows font engine for glyph
        #   metrics at that vanishing scale, GetGlyphOutline() fails with
        #   error 1003 (ERROR_PARTIAL_COPY), which nulls the painter engine
        #   and cascades into every subsequent "Painter not active" warning.
        #   Skipping labels whose projected area is below 1 px² is invisible
        #   to the user (the face itself is already nearly invisible) and
        #   completely prevents the font-engine crash.
        pts = dst
        n = len(pts)
        area = 0.0
        for i in range(n):
        ...
        if abs(area) * 0.5 < self._SIZE * self._SIZE * 0.04:
            return
    ...
    def _draw_gizmo(self, p, pal, D, U, R, opacity):
        ...
        axes = [
            (
                ROT.T @ np.array([1.0, 0.0, 0.0]),
                _qcolor(self._style.gizmo_x_color),
                "X",
            ),
            (
                ROT.T @ np.array([0.0, 1.0, 0.0]),
                _qcolor(self._style.gizmo_y_color),
                "Y",
            ),
            (
                ROT.T @ np.array([0.0, 0.0, 1.0]),
                _qcolor(self._style.gizmo_z_color),
                "Z",
            ),
        ]
    ...
    def _hit(self, pos: QPointF) -> str | None:
        ...
        for _, nm, f in fs:
            if QPolygonF(
                [self._proj(pt, R, U, cx, cy) for pt in f["pts"]]
    ...
    def _install_parent_filter(self) -> None:
        """Install an event filter on the active layout target."""
        p = (
            self._layout_target
            if self._layout_target is not None
    ...
    def eventFilter(self, obj, event):  # noqa: N802
        """Handle layout target changes for sizing and positioning.

        Firing synchronously (before the target widget's own resizeEvent handler)
        means any positioning code in the host app reads the already-updated
        cube dimensions.
        """
        if obj is getattr(self, "_tracked_parent", None) and event.type() in (
            QEvent.Type.Resize,
            QEvent.Type.Move,
            QEvent.Type.Show,
            QEvent.Type.WindowStateChange,
        ):
            if event.type() == QEvent.Type.Resize and self._style.size_fraction > 0.0:
                self._update_dpi()
            self.update_relative_position()
    ...
    def _nearest_face_up(
        self,
        tgt: np.ndarray,
        default_up: np.ndarray,
        face_type: str,
        *,
        cur_dir=None,
        cur_up=None,
    ...
    def _act_face(self, nm: str):
        ...
        up_nav = self._nearest_face_up(
            tgt_nav, default_up, face["ft"], cur_dir=cur_dir_nav, cur_up=cur_up_nav
        )
    ...
    def _act_ctrl(self, act: str):
        ...
        step = self._STEP
        if act == "orbit_u":
            nd, nu = _rod(D, R, -step), _rod(U, R, -step)
        elif act == "orbit_d":
            nd, nu = _rod(D, R, step), _rod(U, R, step)
        elif act == "orbit_l":
            nd, nu = _rod(D, np.array([0.0, 0.0, 1.0]), step), U.copy()
        elif act == "orbit_r":
            nd, nu = _rod(D, np.array([0.0, 0.0, 1.0]), -step), U.copy()
        elif act == "roll_ccw":
            nd, nu = D.copy(), _rod(U, D, -step)
        elif act == "roll_cw":
            nd, nu = D.copy(), _rod(U, D, step)
        elif act == "backside":
            nd, nu = -D.copy(), U.copy()
